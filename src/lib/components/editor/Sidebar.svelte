				...
				{#each children as childId}
					...
					{#if child}
						<button
							...
							class="group flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 pl-6 text-sm hover:bg-accent {editorState.isSelected(child.id)
								? 'bg-accent text-accent-foreground'
								: ''}"
						>
			...
			<div class="space-y-1">
				{#each editorState.constraints as constraint}
					<button
						...
						class="group flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-accent {editorState.isSelected(constraint.id)
							? 'bg-accent text-accent-foreground'
							: ''}"
					>
