<script lang="ts">
	...
	import { editorState, editor } from '$lib/stores/editor.svelte';
...
{#if editor.enabled}
	<div class="flex h-full flex-col bg-background">
		...
		<div class="flex-1 overflow-y-auto p-4">
				...
				<div class="space-y-4">
					<div>
						...
						<p class="mt-1 text-sm">
							{editorState.selectedIds.length} object{editorState.selectedIds.length !== 1
								? 's'
								: ''}
						</p>
					</div>
			...
			{:else if editorState.selectedConstraints && editorState.selectedConstraints.length > 0}
